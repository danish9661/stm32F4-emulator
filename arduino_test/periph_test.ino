...
// DMA2 stream 0: base 0x40026400, stream regs at offset 0x10
...
#define DMA2_S0_FCR  (*(volatile uint32_t*)0x40026424)
...
#define CHECK(cond, msg) do { \
    if (cond) { puts("  PASS "); puts(msg); putc('\n'); pass++; } \
    else { puts("  FAIL "); puts(msg); putc('\n'); fail++; } \
} while(0)
...
void setup() {
    ...
    puts("=== PERIPHERAL TEST ===\n");

    // 1. RCC: read CR (stub returns 0xFFFFFFFF)
    CHECK(RCC_CR == 0xFFFFFFFF, "RCC_CR = 0xFFFFFFFF");
    CHECK(RCC_CFGR == 8, "RCC_CFGR = 8 (SWS=HSI)");

    // 2. SysTick: VAL changes after delay
    STK_CTRL = 1;
    uint32_t sy1 = STK_VAL;
    volatile int d;
    for (d = 0; d < 100; d++);
    uint32_t sy2 = STK_VAL;
    CHECK(sy1 != sy2, "SysTick VAL changed");

    // 3. SPI1: write/read DR
    GPIOA_MODER &= ~((3 << 10) | (3 << 12) | (3 << 14));
    GPIOA_MODER |=  ((2 << 10) | (2 << 12) | (2 << 14));
    GPIOA_AFRL  |=  ((5 << 20) | (5 << 24) | (5 << 28));
    SPI1_CR1 = 0x0347;
    SPI1_DR = 0xA5;
    CHECK(SPI1_SR & 1, "SPI1 TXE set after write");

    // 4. I2C1: write/read registers
    GPIOB_MODER &= ~((3 << 12) | (3 << 14));
    GPIOB_MODER |=  ((2 << 12) | (2 << 14));
    GPIOB_AFRL  |=  ((4 << 24) | (4 << 28));
    I2C1_CR1 = 0x8001;
    I2C1_DR = 0x42;
    CHECK((I2C1_SR1 & 0x3F) == 0, "I2C1_SR1 lower bits = 0");

    // 5. DMA2: memory-to-memory copy via stream 0
    ...
    // MEM2MEM=bit14, PSIZE=32bit=10, MSIZE=32bit=10, MINC=bit10, PINC=bit9, EN=bit0
    for (d = 0; d < 1000; d++);
    CHECK(DMA2_S0_NDTR == 0, "DMA NDTR = 0 after xfer");
    CHECK(dst[0] == 0xDEADBEEF, "DMA dst[0] = 0xDEADBEEF");
    CHECK(dst[1] == 0xCAFEBABE, "DMA dst[1] = 0xCAFEBABE");
    CHECK(dst[2] == 0x12345678, "DMA dst[2] = 0x12345678");
    CHECK(dst[3] == 0x87654321, "DMA dst[3] = 0x87654321");

    // 6. FSMC: control register reads 0, bank1 access returns 0 (no ext device)
    CHECK(FSMC_BCR1 == 0, "FSMC_BCR1 = 0");
    FSMC_DATA = 0x12345678;
    uint32_t fv = FSMC_DATA;
    CHECK(fv == 0 || fv == 0x12345678, "FSMC bank1 read");

    puts("--- SUMMARY ---\n");
    puts("PASS: "); putdec(pass); putc('\n');
    puts("FAIL: "); putdec(fail); putc('\n');
    puts("=== DONE ===\n");
    while (1);
}
