...
Object.assign(Module, {
    ...
    Unicorn: function (arch, mode) {
        ...
        this.hook_add = function (type, user_callback, user_data, begin, end, extra) {
            ...
            // Native in-wasm hook: user_callback is a raw function pointer (number)
            // handed back by get_native_*(). Route it straight to uc_hook_add.
            if (typeof user_callback === 'number') {
                var nhook_ptr = Module._malloc(4);
                var nvalist_ptr = Module._malloc(8);
                Module.setValue(nvalist_ptr, extra || 0, 'i32');
                var nret = Module.ccall('uc_hook_add', 'number',
                    ['pointer', 'pointer', 'number', 'pointer', 'pointer', 'number', 'number', 'pointer'],
                    [handle, nhook_ptr, type, user_callback, 0, BigInt(begin || 0), BigInt(end || 0), nvalist_ptr]);
                Module._free(nvalist_ptr);
                if (nret != Module.ERR_OK) {
                    Module._free(nhook_ptr);
                    throw 'Unicorn.js: Function uc_hook_add failed with code ' + nret + ':\n' + Module.strerror(nret);
                }
                var nhook = { handle: Module.getValue(nhook_ptr, '*'), callback: user_callback, native: true };
                Module._free(nhook_ptr);
                return nhook;
            }
            // Wrap callback
        ...
        this.hook_del = function (hook) {
            ...
            // Native hooks own their function pointer in the module table; do not removeFunction.
            if (!(hook && hook.native)) {
