...
| TIM1, TIM8 (advanced) | Yes | Full | up/down/center, OC/PWM, input capture + host injection, OPM one-pulse (mock `t_honor_pass`), DBGMCU freeze, TRGO trigger routing (MMS reset/update → ITR slaves), UIF/CC IRQs | encoder counting live via `tim_encoder_step` (SMS/polarity/wrap — mock `t_tim_enc`) |
...
| RTC + backup registers | Yes | Full | BCD time, alarms A/B, wakeup exits STOP, temperature pair | WUT + timestamp + smooth cal live (mock `t_rtc_wut_ts`); tamper physics + RDP-adjacent BKPR erase (mock `t_rtc_tamper_phys`) |
