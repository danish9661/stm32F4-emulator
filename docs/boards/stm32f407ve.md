...
| USART1, USART2, USART3, USART6 | Yes | Full | async TX/RX, 64 B RX FIFO, RXNE/ORE, TXE/TC, IRQs | LIN/SC/IrDA enable bits stored + probed, async path unchanged (mock `t_usart_modes`); HW flow control + FE/PE live (mock `t_usart_flow_err`); LIN break + Smartcard NACK + IrDA pulses live (mock `t_usart_protocols`) |
