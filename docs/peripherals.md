...
|---|---|---|---|---|---|---|
| ADC | 633 | Detailed | SR/CR1/CR2/SMPR1-2/JOFR/HTR/LTR/SQR1-3/JSQR/JDR/DR; SWSTART-gated conversion, real sampling-time logic (SMPR lookup), channel values (16/17 = temp/Vref, 18 = Vbat, others LCG pseudo-random), EOC/OVR flags (overrun: new conversion while EOC set latches OVR, DR read clears the pair — mock `t_honor_pass`); analog watchdog (AWDEN/AWDSGL/AWDCH + HTR/LTR compare, AWD flag + IRQ on excursion — mock `t_adc_awd_dma`); DMA request on EOC (CR2 DMA bit stages samples for `adc_take_dma` — mock `t_adc_awd_dma`); shared **ADC_Common @ 0x40012300** — CSR mirrors EOC1/2/3+OVR1/2/3 (read-only), CDR = ADC1.DR\|ADC2.DR<<16, CCR DUAL mode + CDR simultaneous latch (both-ready peek, `adc_dual_latched` probe — mock `t_adc_dual`); `C_ADC` Keil alias unified so the F429 map gets the block | Y (EOC/OVR/AWD → 18/47) | N | N |
...
| TIM | 694 | Detailed | CR1/CR2/SMCR/DIER/SR/EGR/CCMR1-3/CCER/CNT/PSC/ARR/CCR1-6/RCR/DCR/DMAR/OR; instruction-count counter with PSC prescaler, up/down/center-aligned, UIF+UIE IRQ on overflow, CC match IRQs, UG update event, PWM duty; OPM one-pulse (CEN self-clears at update in every counting mode — mock `t_honor_pass`); master/slave trigger routing (CR2 MMS reset/update → ITR slaves SMS reset/gated/trigger, deferred end-of-tick drain — mock `t_trgo`); DBGMCU freeze stops advance while halted+frozen; encoder counting live via `tim_encoder_step` (SMS/polarity/wrap — mock `t_tim_enc`) | Y (per-timer) | Y | N |
| USART | 434 | Detailed | SR/DR/BRR/CR1-3/GTPR; TX pushes to global UART output buffer (drained by `get_uart_output`), RX via `rx_byte` (JS `uart_rx_byte`) with 64-byte RX buffer, RXNE/ORE/IDLE (harness `uart_idle`, IDLEIE IRQ, DR-read clear), TXE/TC always set; TX break via SBK (queued flag, self-clearing CR1 bit, consumed by DR write, `uart_break_pending` probe); IRQs on RXNEIE/TCIE/TXEIE/IDLEIE/PEIE/LBDIE (PE is PEIE-gated, not EIE — mock `t_honor_pass`); HW flow control (CTSE gates TX on harness CTS, TXE/TC clear while held) + FE/PE fault injection (PCE-gated PE, DR-read clear) (mock `t_usart_flow_err`); LIN break (LBD + RXNE/0x00, LBDIE IRQ — mock `t_usart_protocols`), Smartcard T=0 NACK retry loop (TC held, 8-cap, NE on exhaustion — mock `t_usart_protocols`), IrDA pulse-class envelope (NE on mismatch — mock `t_usart_protocols`); LIN/SC/IrDA enable bits stored + probed (`lin_active`/`sc_active`/`irda_active`), async path unchanged (mock `t_usart_modes`) | Y (37/38/39/52/53/71/82/83) | N | N |
