...
| CAN | 785 | Detailed | MCR/MSR/TSR/RF0R/RF1R/IER/ESR/BTR; 3 TX + 2 RX mailboxes, filter banks (FMR/FM1R/FS1R/FFA1R/FA1R + 56 filter words), TX request → TXOK/RQCP, RX FIFO release decrements FMP, INIT/SLEEP transitions; **two-node bus arbitration** (lowest ID wins, ties by node/mailbox), winner broadcasts to every RX passing its filters (including itself), BTR LBKM loopback delivers only to the sender; TEC/REC stay 0, BOFF/EPVF/EWGF/LEC never set (mock `t_can_err`); CAN FD via `can_inject_fd` (DLC table, FDF, 64B window 0x320/0x3A0, RFOM shift — mock `t_canfd`) + FDCAN bit-timing NBTP/DBTP @0x3E0/0x3E4 with nominal/data `can_fd_cost` split (mock `t_fdcan_timing`; bxCAN has no FD regs, protocol timing beyond rates unmodeled) | Y (TX/RX/SCE → 19/63) | N | N |
