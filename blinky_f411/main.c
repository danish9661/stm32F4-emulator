// Blinky for BlackPill F411CE (STM32F411CE: 512K flash, 128K RAM).
...
static void led_init(void) {
    ...
    GPIOC_MODER = (GPIOC_MODER & ~(3u << (LED_PIN * 2))) | (1u << (LED_PIN * 2)); // PC13 output
    GPIOC_ODR &= ~(1u << LED_PIN);
}

static void led_set(int on) {
    if (on) GPIOC_ODR |= (1u << LED_PIN);
    else    GPIOC_ODR &= ~(1u << LED_PIN);
}
