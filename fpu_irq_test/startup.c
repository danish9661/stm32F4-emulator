...
// Minimal table: SysTick_Handler in slot 15. Plain C handler (no
// `interrupt` attribute — it warns under hard-float); stacking is exact
...
void (* const vector_table[16])(void) = {
    ...
    [14] = Default_Handler, [15] = SysTick_Handler,
};
