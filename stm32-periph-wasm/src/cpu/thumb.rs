...
pub fn exec16(cpu:&mut Cpu,_sys:&WasmSystem,mem:&mut dyn Memory,op:u16,pc:u32)->bool{
    ...
    if (op&0xF800)==0x2800{ let rn=((op>>8)&7)as usize; let imm=(op&0xFF)as u32; let a=cpu.regs.r[rn]; let r=a.wrapping_sub(imm); cpu.regs.xpsr = (cpu.regs.xpsr & !0xF0000000) | if r==0 {0x40000000} else {0} | if (r as i32)<0 {0x80000000} else {0} | if a>=imm {0x20000000} else {0}; cpu.regs.r[15]=pc.wrapping_add(2)|1; return true; }
    ...
    if (op&0xF000)==0xD000 && (op&0x0F00)!=0x0E00{ let cond=((op>>8)&0xF)as u32; let imm=sx(((op&0xFF)as u32)<<1,9); let take=match cond{0=> (cpu.regs.xpsr&0x40000000)!=0, 1=> (cpu.regs.xpsr&0x40000000)==0, 2=> (cpu.regs.xpsr&0x20000000)!=0, 3=> (cpu.regs.xpsr&0x20000000)==0, 4=> (cpu.regs.xpsr&0x80000000)!=0, 5=> (cpu.regs.xpsr&0x80000000)==0, 10=> (cpu.regs.xpsr&0x80000000)==(cpu.regs.xpsr&0x10000000), 11=> (cpu.regs.xpsr&0x80000000)!=(cpu.regs.xpsr&0x10000000), 12=> (cpu.regs.xpsr&0x40000000)!=0 && (cpu.regs.xpsr&0x80000000)==(cpu.regs.xpsr&0x10000000), 13=> (cpu.regs.xpsr&0x40000000)==0 || (cpu.regs.xpsr&0x80000000)!=(cpu.regs.xpsr&0x10000000), 14=> true, _=> true}; if take { cpu.regs.r[15]=pc.wrapping_add(2).wrapping_add(imm)|1; } else { cpu.regs.r[15]=pc.wrapping_add(2)|1; } return true; }
