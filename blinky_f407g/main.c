// Blinky for Discovery F407VG (PD12 green LED).
...
static void led_init(void) {
    ...
    GPIOD_MODER = (GPIOD_MODER & ~(3u << (LED_PIN * 2))) | (1u << (LED_PIN * 2)); // D12 output
    GPIOD_ODR &= ~(1u << LED_PIN);
}

static void led_set(int on) {
    if (on) GPIOD_ODR |= (1u << LED_PIN);
    else    GPIOD_ODR &= ~(1u << LED_PIN);
}
