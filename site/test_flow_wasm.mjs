...
while (true) {
    const res = emu.step();
    { const fi = emu.faultInfo ? emu.faultInfo() : null; if (fi || res.stopped) console.log(`step ${steps}: pc=0x${(res.pc>>>0).toString(16)} stopped=${res.stopped} fault=${JSON.stringify(fi)} inst=${res.instCount}`); }
    if (res.stopped) { const r = emu.getRegisters(); console.log(`STOPPED at step ${steps} pc=0x${(res.pc>>>0).toString(16)} fault=${JSON.stringify(emu.faultInfo())}`); break; }
    steps++;
    inst = res.instCount;
    uart += emu.drainUart();
