...
import * as bindings from './vendor/stm32_periph_wasm.js?v=36';
...
const IRQ_FIRMWARES = new Set(['rx_interrupt_test', 'rx_interrupt_test_f401', 'rx_interrupt_test_f411', 'rx_interrupt_test_f429', 'rx_crypto_test', 'comprehensive_test', 'comprehensive_test_f429', 'dma2d_test', 'dma2d_test_f429', 'eth_irq_test', 'edge_test', 'periph_test', 'fpu_irq_test', 'mpu_test', 'exti_test', 'freertos_test',
...
const boot = async () => {
    ...
    if (bridgeUrl) {
        ...
        // NOTE (VENDOR_V): vendor asset versions (?v=36) must be bumped together
        ...
        const svdXml = await fetch('vendor/' + board.svd + '?v=36').then((r) => r.text());
        ...
        emu = await createEmulator({
            ...
            wasmUrl: 'vendor/stm32_periph_wasm_bg.wasm?v=36',
...
if (preset && FIRMWARES[preset]) {
    ...
    // Both setters, then verify: __setFwSelect silently no-ops when the
    // preset has no dropdown entry (missing option), and btnBoot boots
    // whatever fwSelect holds (default eth_http) — exactly the silent
    // wrong-firmware boot standby_demo hit (no console.html entry).
    if (window.__setFwSelect) window.__setFwSelect(preset);
    $('fwSelect').value = preset;
    if ($('fwSelect').value !== preset) {
        setStatus(`unknown preset ?fw=${preset} (not in dropdown)`, 'err');
        appendUart(`Unknown preset "${preset}" — not in the firmware dropdown. `
            + `Pick one under FIRMWARE (right) and press "Boot preset".\r\n`);
    } else {
        $('btnBoot').click();
    }
}
