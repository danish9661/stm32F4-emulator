...
import * as bindings from './vendor/stm32_periph_wasm.js?v=6';
...
const boot = async () => {
    ...
    if (bridgeUrl) {
        ...
        // NOTE (VENDOR_V): vendor asset versions (?v=6) must be bumped together
        // after every wasm-pack rebuild, or browsers keep the stale model.
        const svdXml = await fetch('vendor/stm32f407.svd?v=6').then((r) => r.text());
        ...
        emu = await createEmulator({
            ...
            bindings,
            svdXml,
            wasmUrl: 'vendor/stm32_periph_wasm_bg.wasm?v=6',
