...
|---|---|---|---|---|---|---|
| ADC | 517 | Detailed | SR/CR1/CR2/SMPR1-2/JOFR/HTR/LTR/SQR1-3/JSQR/JDR/DR; SWSTART-gated conversion, real sampling-time logic (SMPR lookup), channel values (16/17 = temp/Vref, 18 = Vbat, others LCG pseudo-random), EOC/OVR flags; analog watchdog (AWDEN/AWDSGL/AWDCH + HTR/LTR compare, AWD flag + IRQ on excursion); DMA request on EOC (CR2 DMA bit stages samples for `adc_take_dma`); shared **ADC_Common @ 0x40012300** — CSR mirrors EOC1/2/3+OVR1/2/3 (read-only), CDR = ADC1.DR\|ADC2.DR<<16, CCR stored (dual-mode config accepted, never interleaved); `C_ADC` Keil alias unified so the F429 map gets the block | Y (EOC/OVR/AWD → 18/47) | N | N |
| CAN | 700 | Detailed | MCR/MSR/TSR/RF0R/RF1R/IER/ESR/BTR; 3 TX + 2 RX mailboxes, filter banks (FMR/FM1R/FS1R/FFA1R/FA1R + 56 filter words), TX request → TXOK/RQCP, RX FIFO release decrements FMP, INIT/SLEEP transitions; **two-node bus arbitration** (lowest ID wins, ties by node/mailbox), winner broadcasts to every RX passing its filters (including itself), BTR LBKM loopback delivers only to the sender; TEC/REC stay 0, BOFF/EPVF/EWGF/LEC never set (mock `t_can_err`); CAN FD via `can_inject_fd` (DLC table, FDF, 64B window 0x320/0x3A0, RFOM shift — mock `t_canfd`; bxCAN has no FD regs, FDCAN-class timing unmodeled) | Y (TX/RX/SCE → 19/63) | N | N |
...
| DAC | 136 | Detailed | CR/SWTRIGR/all DHR regs/DOR1-2/SR; DOR update on trigger + writes (register model real — mock `t_dac_sink`), LFSR noise, triangle-waveform counter/direction, MAMP masks; no analog pin sink (PA4 IDR unaffected) | N | N | N |
...
| DCMI | 479 | Detailed | CR/SR/RIS/IER/MIS/ICR/ESCR/ESUR/CWSTRT/CWSIZ/DR + JPEG/ESS/CROP bytes; full camera pipeline (VSYNC/LINE/FRAME/ERR/OVR, CROP window, JPEG word-pack, FIFO, IRQ78) fed by `dcmi_feed_frame` (JS camera); guest coverage: `dcmi_test` | Y (78) | Y | Y |
...
| ETH | 2530 | Detailed | All 4 blocks (MAC/MMC/PTP/DMA) full register maps incl. WOL wakeup-filter CRC, addend drift, PPS counter/level, VLAN gate+invert, PTP snapshots, TX checksum insert, RX checksum status, accept filtering (perfect/hash/promiscuous/DAIF/ROD), wire pacing, deferral/collision reports, RBUS head-only RX; DP83848-style PHY/MDIO peer; MII/RMII pin mirrors in GPIO IDR; BSD sockets over real LwIP; PPS observed via counter+level (not the pin — mock `t_pps`), nibble data + 25/50 MHz clocks electrical-only (level mirrors are the contract — mock `t_nibble`), no ULPI PHY (viewport benign-0 — mock `t_ulpi`); guest coverage: `eth_feat_test` (60+ markers, both maps), `eth_http/dhcp/test/irq/pins`, `lwip_demo` | Y (61/62) | Y | Y |
...
| FLASH | 149 | Detailed | ACR, KEYR two-step unlock (0x45670123→0xCDEF89AB), OPTKEYR, SR write-1-clear, CR + LOCK/PSIZE, OPTCR1; program (PG) and sector-erase (SER) dispatch into the emulated flash backing buffer via `flash_erase_applied` (JS driver gated on `syncFlashProtection`) | N | N | N |
| FSMC | 374 | Detailed | 4 banks 0x60000000-0xA0001000; BCR/BTR/BWTR + NAND PCR/SR/PMEM/PATT/ECCR with a REAL ECC engine (24-bit order-sensitive parity over data-space writes while ECCEN set, reset on ECCEN rise, frozen while clear; vendor Hamming matrix proprietary so the code differs from silicon bit-for-bit — the round-trip contract is what firmware checks); data-space accesses forward to a JS device via the bank tap (`fsmc_tap` / `ext_devices.fsmcDevices`), which reports the ACCESS ADDRESS as well as the value so an 8080-mode display can decode its RS/DC line. An untapped bank reads 0; FMC alias unified so the Keil F429 map gets the block; guest coverage: `fsmc_test` | N | N | Y |
...
| GPIO | 272 | Detailed | MODER/OTYPER/OSPEEDR/PUPDR/IDR/ODR/BSRR/LCKR/AFRL/AFRH; ODR/BSRR drive output_state + write callbacks, MODER→input clears output, IDR = input_state + read callbacks, 11 ports (A-K) | N | N | Y |
| HASH | 165 | Detailed | Real SHA-1/MD5/SHA-256 (sha1/md5/sha2 crates) with NBLW/length handling; CR/INIT, DIN FIFO, STR, HR + HASH_HR, IMR/SR, 54 CSR words; HMAC MODE/LKEY store verbatim but never alter the digest (mock `t_hash`) | Y (80) | N | N |
| I2C | 466 | Detailed | Full master state machine (Idle→Start→Addr→Active read/write), START/STOP, SWRST, address match vs attached EEPROMs (NACK/AF on miss), byte R/W, SR1/SR2 ordering semantics, event/buffer/error IRQ masking; SMBus PEC (CRC-8, ENPEC-gated, PECERR — mock `t_i2c_smbus`), ARLO on armed loss (harness = other master), TIMEOUT after 32 Active ticks with SMBUS set; single-master only otherwise (bus always won — mock `t_i2c_multi`) | Y (31-34, 72-73) | Y | Y |
...
| RCC | 227 | Detailed | CR/PLLCFGR/CFGR/CIR/reset-enable/low-power/BDCR/CSR/SSCG/PLLI2SCFGR/PLLSAI/DCKCFGR/CKGATENR/DCKCFGR2; HSE/PLL ready after instruction-count delays (RDY clears at once when ON drops — mock `t_rcc`), HSIRDY, SWS mirrors SW, real freq math (system/pll/ahb/apb1/apb2), enable-bit gating map, LSE/LSI ready timing; sources always lock (no fail injection) | N | N | N |
| RNG | 131 | Detailed | CR/SR/DR @0x50060800; LCG regenerated every 40 inst on tick (RNGEN-gated) + on DR read, DRDY, error flags + IRQ; host entropy pool via `rng_seed_entropy` (FIFO, one/regen, SECS reports LCG-fallback — mock `t_entropy`); unseeded = deterministic LCG by design (mock `t_rng`) | Y (80) | Y | N |
| RTC | 199 | Detailed | TR/DR/CR/ISR/PRER/WUTR/CALIBR/ALRMAR/ALRMBR/WPR/SSR/SHIFTR/timestamp/CALR/TAFCR/ALRMASSR/ALRMBSSR + 20 backup regs; BCD time advances via PRER prescaler vs instruction count, alarm A/B matching with don't-care masks | Y (41) | N (advances on access) | N |
...
| SDIO | 144 | Detailed | Full register set + emulated SD card state machine (Idle→Ident→Stby→Tran), canned responses for CMD0/2/3/5/7/8/9/10/13/16/17/18/41/55, RCA matching, data-transfer simulation (DCOUNT/FIFOCNT, CMD17/18), status flags + ICR/MASK | Y (49) | N | N |
| SPI | 319 | Detailed | CR1/CR2/SR/DR/RXCRC/TXCRC/I2SCFGR/I2SPR; full-duplex 8/16-bit transfers to attached device with CS selection via GPIO, I2S mode audio generation, TXE/RXNE toggling; **CS edges delivered via GPIO write callbacks** (`register_cs_callbacks`, sw_spi pattern) so attached-device CS deassert is observed immediately; master-only — MSTR=0 stores but transfers still run the master path (mock `t_spi_slave`) | Y (35/36/51) | N | Y |
