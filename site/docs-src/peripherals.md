...
| USART | 434 | Detailed | SR/DR/BRR/CR1-3/GTPR; TX pushes to global UART output buffer (drained by `get_uart_output`), RX via `rx_byte` (JS `uart_rx_byte`) with 64-byte RX buffer, RXNE/ORE/IDLE (harness `uart_idle`, IDLEIE IRQ, DR-read clear), TXE/TC always set; TX break via SBK (queued flag, self-clearing CR1 bit, consumed by DR write, `uart_break_pending` probe); IRQs on RXNEIE/TCIE/TXEIE/IDLEIE/PEIE/LBDIE (PE is PEIE-gated, not EIE — mock `t_honor_pass`); HW flow control (CTSE gates TX on harness CTS, TXE/TC clear while held) + FE/PE fault injection (PCE-gated PE, DR-read clear) (mock `t_usart_flow_err`); IRQ on RXNEIE/TCIE/TXEIE; LIN break (LBD + RXNE/0x00, LBDIE IRQ — mock `t_usart_protocols`), Smartcard T=0 NACK retry loop (TC held, 8-cap, NE on exhaustion — mock `t_usart_protocols`), IrDA pulse-class envelope (NE on mismatch — mock `t_usart_protocols`); LIN/SC/IrDA enable bits stored + probed (`lin_active`/`sc_active`/`irda_active`), async path unchanged (mock `t_usart_modes`) | Y (37/38/39/52/53/71/82/83) | N | N |
