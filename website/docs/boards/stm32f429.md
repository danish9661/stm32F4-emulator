...
| SPI1–SPI6 + I2S1–3 | Yes | Full | master 8/16-bit, CS callbacks, flash/tap slaves, WAV-backed audio + TX capture | slave-select stored + probed, master path runs (mock `t_spi_slave`); HW CRC regs stored, not computed |
