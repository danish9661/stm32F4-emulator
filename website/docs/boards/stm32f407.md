...
| RTC + backup registers | Yes | Full | BCD time, alarms A/B, wakeup exits STOP, temperature pair | WUT + timestamp + smooth cal live (mock `t_rtc_wut_ts`); tamper physics + RDP-adjacent BKPR erase (mock `t_rtc_tamper_phys`); tamper-pin physics N/A |
