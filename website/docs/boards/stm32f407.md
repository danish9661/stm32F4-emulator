...
## Demos on this chip (72 presets)

`adc_demo`, `arduino_black_f407ve`, `arduino_black_f407ze`, `arduino_disco_f407vg`, `arduino_test`, `arduino_test_disco_f407vg`, `audio_play_test`, `audio_test`, `blink_serial`, `blinky`, `blinky_f407g`, `blinky_f407ve`, `blinky_f407ze`, `buzzer_test`, `can_demo`, `can_host_rx`, `can_test`, `comprehensive_test`, `crypto_deep_test`, `crypto_test`, `crypto_test_disco_f407vg`, `dac_demo`, `dcmi_test`, `deep_periph_test`, `deep_sleep_demo`, `standby_demo`, `echo_test`, `echo_test_disco_f407vg`, `edge_test`, `edge_test_disco_f407vg`, `eth_adv`, `eth_feat_test`, `eth_irq_test`, `eth_pins_test`, `exti_test`, `flash_test`, `fpu_irq_test`, `fpu_test`, `freertos_test`, `fsmc_test`, `hal_test`, `hal_test_disco_f407vg`, `i2s_sai_test`, `ltdc_test`, `lwip_demo`, `mpu_test`, `new_periph_test`, `oled_test`, `periph_test`, `periph_test_disco_f407vg`, `pwm_demo`, `qspi_test`, `gap10_sdio`, `gap10_qspi`, `gap10_ltdc`, `gap10_i2c`, `gap10_dma`, `gap10_dac`, `rtc_test`, `rx_crypto_test`, `rx_interrupt_test`, `spi_flash_test`, `spi_tft_test`, `test_firmware`, `tft_test`, `tim_capture_demo`, `timer_test`, `timer_test_disco_f407vg`, `usb_cdc_test`, `watchdog_demo`, `wwdg_demo`, `wwdg_window_demo`.

## Verification

- `node site/test_arduino_boards.mjs` — Discovery build PASS
- `node site/test_board_matrix.mjs` — all 72 entries green
...
| UART4, UART5 | Yes | Full | same USART model | LIN/SC/IrDA stored-only (mock `t_usart_modes`); FE/PE + CTSE live (mock `t_usart_flow_err`); LIN/SC/IrDA protocols live (mock `t_usart_protocols`) |
...
| SDIO | Yes | Full | card state machine, CMD0/2/3/5/7/8/9/10/13/16/17/18/41/55, block reads, IRQ49 | ACMD prefix + wide-bus + DAT1 IRQ live (mock `t_sdio_acmd`); width-scaled timing + DTIMEOUT/DCRCFAIL (mock `t_sdio_timing`) |
...
| RTC + backup registers | Yes | Full | BCD time, alarms A/B, wakeup exits STOP, temperature pair | WUT + timestamp + smooth cal live (mock `t_rtc_wut_ts`); tamper physics + RDP-adjacent BKPR erase (mock `t_rtc_tamper_phys`), SHIFTR shift + ALRMASSR SS gate (mock `t_gap9`); tamper-pin physics N/A |
