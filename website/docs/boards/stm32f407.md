...
| TIM1, TIM8 (advanced) | Yes | Full | up/down/center, OC/PWM, input capture + host injection, DBGMCU freeze, UIF/CC IRQs | encoder-mode counting; timer-to-timer trigger routing |
| TIM2–TIM5 (general-purpose) | Yes | Full | same model (32-bit TIM2/5) | same gaps |
| TIM6, TIM7 (basic) | Yes | Full | time-base + UIF IRQ, DBGMCU freeze | — |
| TIM9–TIM14 | Yes | Full | time-base + capture/compare + IRQs, DBGMCU freeze | — |
...
| DCMI | Yes | Partial | JS-fed frames, 4-deep FIFO, VSYNC/LINE/FRAME/ERR/OVR + IRQ78, MIS masked status, CROP window via CWSTRT/CWSIZE | pin-sync sampling; JPEG mode |
| FSMC | Yes | Partial | 4 banks, BCR/BTR/BWTR + NAND PCR/SR/PMEM/PATT/ECCR, JS bank taps (address + value) | access timings / wait states; NAND ECC never computed (no array); untapped banks read 0 |
...
| DBGMCU | Yes | Partial | IDCODE 0x10006411; APBx freeze honored — halted+frozen TIM2-14/WWDG/IWDG neither count nor fire (`dbgmcu_set_halt`, no catch-up burst) | no debugger attached, so halt is a test/driver state |
