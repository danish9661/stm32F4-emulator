...
| USART1, USART2, USART3, USART6 | Yes | Full | async TX/RX, 64 B RX FIFO, RXNE/ORE, TXE/TC, IRQs | LIN/SC/IrDA enable bits stored + probed, async path unchanged (mock `t_usart_modes`); HW flow control live (CTSE gates TX on CTS — mock `t_usart_flow_err`); FE/PE fault injection (mock `t_usart_flow_err`) |
| UART4, UART5 | Yes | Full | same USART model | LIN/SC/IrDA stored-only (mock `t_usart_modes`); FE/PE + CTSE live (mock `t_usart_flow_err`) |
| SPI1, SPI2, SPI3 + I2S1–3 | Yes | Full | master 8/16-bit, CS callbacks, flash/tap slaves, WAV-backed audio + TX capture | slave-select stored + probed, master path runs (mock `t_spi_slave`); HW CRC live (CRCPR poly, RXCRCR/TXCRCR, CRCNEXT/CRCERR) + OVR/MODF/FRE/BSY (mock `t_spi_crc_err`) |
...
| SDIO | Yes | Full | card state machine, CMD0/2/3/5/7/8/9/10/13/16/17/18/41/55, block reads, IRQ49 | ACMD prefix + wide-bus + DAT1 IRQ live (mock `t_sdio_acmd`); bus-width data timing identical by design |
...
| RTC + backup registers | Yes | Full | BCD time, alarms A/B, wakeup exits STOP, temperature pair | WUT + timestamp + tamper + smooth cal live (mock `t_rtc_wut_ts`); tamper-pin physics N/A |
