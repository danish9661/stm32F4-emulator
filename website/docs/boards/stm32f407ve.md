...
| USART1, USART2, USART3, USART6 | Yes | Full | async TX/RX, 64 B RX FIFO, RXNE/ORE, TXE/TC, IRQs | LIN/SC/IrDA enable bits stored + probed, async path unchanged (mock `t_usart_modes`); HW flow control |
| UART4, UART5 | Yes | Full | same USART model | LIN/SC/IrDA stored-only (mock `t_usart_modes`) |
| SPI1, SPI2, SPI3 + I2S1–3 | Yes | Full | master 8/16-bit, CS callbacks, flash/tap slaves, WAV-backed audio + TX capture | slave-select stored + probed, master path runs (mock `t_spi_slave`); HW CRC regs stored, not computed |
