...
| USART | 154 | Detailed | SR/DR/BRR/CR1-3/GTPR; TX pushes to global UART output buffer (drained by `get_uart_output`), RX via `rx_byte` (JS `uart_rx_byte`) with 64-byte RX buffer, RXNE/ORE, TXE/TC always set; IRQ on RXNEIE/TCIE/TXEIE; LIN/SC/IrDA enable bits stored + probed (`lin_active`/`sc_active`/`irda_active`), async path unchanged (mock `t_usart_modes`) | Y (37/38/39/52/53/71/82/83) | N | N |
