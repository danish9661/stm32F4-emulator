// CAN bus demo: a friendly, self-contained showcase of the dual-node CAN
// model. Demonstrates (1) single-controller loopback TX/RX, and (2) two
// controllers (CAN1, CAN2) sharing one bus where the lower arbitration ID
// wins and the frame is broadcast to both nodes.
// Usage: flash can_demo.bin (or select "can_demo" in the web demo), open the
// UART terminal. Expect:
//   === CAN Demo ===
...
// Drain every pending FIFO0 frame (mailboxes 0..FMP-1, oldest first) and
// release each. Returns the number of frames read.
...
int main(void) {
    ...
    // ===== Demo 1: loopback (LBKM) on a single controller =====
    uart_puts("Demo 1: CAN1 loopback\r\n");
    ...
    // ===== Demo 2: two nodes share the bus; lower ID wins =====
    uart_puts("Demo 2: CAN1 + CAN2 arbitration\r\n");
