...
pub struct System<'a, 'b> {
    ...
    pub d: Rc<ExtDevices>,
    pub n: Option<Rc<RefCell<Netlist>>>,
}
...
impl<'a, 'b> System<'a, 'b> {
    ...
    fn bind_peripherals_to_unicorn(&mut self, netlist: Option<Rc<RefCell<Netlist>>>) -> Result<()> {
        for (start, end) in Peripherals::MEMORY_MAPS {
            let n_read = netlist.clone();
            let p = self.p.clone();
            let d = self.d.clone();
            let read_cb = move |uc: &mut Unicorn<'_, ()>, addr, size| {
                let mut sys = System { uc: RefCell::new(uc), p: p.clone(), d: d.clone(), n: n_read.clone() };
                p.read(&mut sys, start + addr as u32, size as u8) as u64
            };

            let n_write = netlist.clone();
            let p = self.p.clone();
            let d = self.d.clone();
            let write_cb = move |uc: &mut Unicorn<'_, ()>, addr, size, value| {
                let mut sys = System { uc: RefCell::new(uc), p: p.clone(), d: d.clone(), n: n_write.clone() };
...
-> Result<(System<'a, 'b>, Framebuffers)>
  {
    ...
    let netlist = config.components.map(|comps| {
        let mut nl = Netlist::new();
        for comp in comps {
            nl.add_component(comp.into_component());
        }
        if let Some(ref wires) = config.wires {
            nl.build_wires(wires);
        }
        Rc::new(RefCell::new(nl))
    });
    ...
    system.n = netlist.clone();
    system.bind_peripherals_to_unicorn(netlist)?;
