...
impl Iwdg {
    ...
    fn decrement_counter(&mut self) {
        ...
        if self.sr != 0 && now.saturating_sub(self.sr_tick) >= self.tick_instructions() {
            self.sr = 0;
        }
        ...
        match offset {
            ...
            0x04 => {
                if self.kr == 0x5555 {
                    self.pr = value & 7;
                    self.sr |= 1;
                    self.sr_tick = crate::emulator::NUM_INSTRUCTIONS.load(Ordering::Relaxed);
                }
            }
            0x08 => {
                if self.kr == 0x5555 {
