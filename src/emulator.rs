...
pub fn run_emulator(config: Config, svd_device: SvdDevice, args: Args) -> Result<()> {
    ...
    // The slowdown is less than 50%. It's okay for now.
    {
        ...
        let p = sys.p.clone();
        let d = sys.d.clone();
        let n_netlist = sys.n.clone();
        let interrupt_period = args.interrupt_period;
        let emit_interval = args.emit_state;
        ...
        sys.uc.borrow_mut().add_code_hook(0, u64::MAX, move |uc, pc, size| {
            ...
            if n % interrupt_period as u64 == 0 {
                let sys = System { uc: RefCell::new(uc), p: p.clone(), d: d.clone(), n: n_netlist.clone() };
                p.nvic.borrow_mut().run_pending_interrupts(&sys, vector_table_addr);
                if let Some(ref nl) = sys.n {
                    nl.borrow_mut().tick(&sys);
                }
            }
            ...
            if let Some(interval) = emit_interval {
                if n % interval == 0 {
                    let sys = System { uc: RefCell::new(uc), p: p.clone(), d: d.clone(), n: n_netlist.clone() };
                    let gpio_b = sys.p.gpio.borrow();
                    let state = crate::state::collect_state(&sys, &gpio_b, n);
...

    {
        let p = sys.p.clone();
        let d = sys.d.clone();
        let n_intr = sys.n.clone();
        sys.uc.borrow_mut().add_intr_hook(move |uc, exception| {
            match exception {
                8 => {
                    let sys = System { uc: RefCell::new(uc), p: p.clone(), d: d.clone(), n: n_intr.clone() };
